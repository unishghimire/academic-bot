...
import { initHourlyMotivationJob } from './jobs/hourly-motivation.job.js';
...
export function createDiscordClient() {
    ...
    client.once(Events.ClientReady, async (readyClient) => {
        ...
        for (const [guildId, guild] of readyClient.guilds.cache) {
            if (guildId !== env.DISCORD_GUILD_ID) {
                logger.warn({ guildId, guildName: guild.name, targetGuildId: env.DISCORD_GUILD_ID }, 'Leaving unauthorized server to enforce dedicated single-server restriction.');
                try {
                    await guild.leave();
                }
                catch (err) {
        ...
        if (targetGuild) {
            logger.info({ guildId: targetGuild.id, guildName: targetGuild.name }, 'Connected to authorized server. Deploying slash commands...');
            ...
            logger.warn({ targetGuildId: env.DISCORD_GUILD_ID, inviteUrl }, '⚠️ Bot is NOT in the authorized server yet! Please invite the bot using the URL above.');
        }
        ...
        initHourlyMotivationJob(client);
    });
    // Automatically enforce single-server restriction upon new guild join
    client.on(Events.GuildCreate, async (guild) => {
        if (guild.id !== env.DISCORD_GUILD_ID) {
            logger.warn({ guildId: guild.id, guildName: guild.name, targetGuildId: env.DISCORD_GUILD_ID }, 'Bot joined an unauthorized server. Leaving immediately to enforce single-server restriction.');
            try {
                await guild.leave();
            }
            catch (err) {
        ...
        logger.info({ guildId: guild.id, guildName: guild.name }, '🎉 Bot was added to designated authorized server! Synchronizing slash commands...');
        try {
            await deployCommands();
    ...
    client.on(Events.InteractionCreate, async (interaction) => {
        ...
        if (interaction.guildId && interaction.guildId !== env.DISCORD_GUILD_ID) {
            logger.warn({ guildId: interaction.guildId, targetGuildId: env.DISCORD_GUILD_ID }, 'Ignored interaction from unauthorized server.');
