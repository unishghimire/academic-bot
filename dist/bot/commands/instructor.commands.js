...
exports.studentProgressCommand = exports.projectReviewCommand = exports.assignmentReviewCommand = void 0;
...
exports.assignmentReviewCommand = {
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .setRequired(true)
        .addChoices({ name: 'Approve (+150 XP)', value: 'APPROVED' }, { name: 'Reject (Needs Work)', value: 'REJECTED' }))
    ...
    async execute(interaction) {
        ...
        const submission = await client_js_1.prisma.assignmentSubmission.findUnique({
            where: { id: submissionId },
            include: { assignment: true, user: true },
        });
        if (!submission) {
            await interaction.editReply({
                embeds: [(0, embed_builder_js_1.createWarningEmbed)('Submission Not Found', `No assignment submission with ID \`${submissionId}\`.`)],
            });
            return;
        }
        const previousStatus = submission.status;
        await client_js_1.prisma.assignmentSubmission.update({
            where: { id: submissionId },
            data: {
                status: decision,
        ...
        if (decision === 'APPROVED') {
            await xp_service_js_1.xpService.awardXp(submission.userId, constants_js_1.XP_REWARDS.ASSIGNMENT_APPROVED, `Assignment Approved: ${submission.assignment.title}`, 'assignment', submissionId);
            // Re-evaluate lesson completion
            await progress_service_js_1.progressService.evaluateLessonCompletion(submission.userId, submission.assignment.lessonId);
        }
            ...
            try {
                ...
                if (studentUser) {
                    ...
                    const studentFeedbackEmbed = isApproved
                        ? (0, embed_builder_js_1.createSuccessEmbed)(`🎉 Assignment Approved: ${submission.assignment.title}`, `Your assignment submission has been evaluated by an instructor.\n\n` +
                            `• **Status:** **APPROVED**\n` +
                            `• **Instructor Feedback:**\n> *${feedback}*\n\n` +
                            `• **XP Earned:** **+150 XP**\n` +
                            `• **Reviewed by:** <@${interaction.user.id}>\n\n` +
                            `Run \`/progress\` or \`/course next\` in the server to view your next lesson!`)
                        : (0, embed_builder_js_1.createWarningEmbed)(`📝 Assignment Feedback: ${submission.assignment.title}`, `Your instructor has reviewed your assignment submission and requested revisions.\n\n` +
        ...
        await interaction.editReply({
            embeds: [
                (0, embed_builder_js_1.createSuccessEmbed)('Assignment Review Recorded', `Submission \`${submissionId}\` marked as **${decision}**.\nFeedback sent to student: *${feedback}*`),
            ],
        });
    },
};
exports.projectReviewCommand = {
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .setDescription('Grading decision')
        .setRequired(true)
        .addChoices({ name: 'Approve (+500 XP)', value: 'APPROVED' }, { name: 'Needs Revision', value: 'NEEDS_REVISION' }, { name: 'Reject', value: 'REJECTED' }))
    ...
    async execute(interaction) {
        ...
        const decision = interaction.options.getString('decision', true);
        const feedback = interaction.options.getString('feedback', true);
        const submission = await client_js_1.prisma.projectSubmission.findUnique({
            where: { id: submissionId },
            include: { project: true, user: true },
        });
        if (!submission) {
            await interaction.editReply({
                embeds: [(0, embed_builder_js_1.createWarningEmbed)('Not Found', `No project submission with ID \`${submissionId}\`.`)],
            });
            return;
        }
        await client_js_1.prisma.projectSubmission.update({
            where: { id: submissionId },
            data: {
                status: decision,
                reviewedBy: interaction.user.id,
        ...
        if (decision === 'APPROVED') {
            await xp_service_js_1.xpService.awardXp(submission.userId, constants_js_1.XP_REWARDS.PROJECT_APPROVED, `Tier ${submission.project.tier} Final Project Approved!`, 'project', submissionId);
            // Trigger central Tier Unlock Engine!
            const tierResult = await tier_engine_service_js_1.tierEngine.evaluateTier(submission.userId);
            // If a new tier was unlocked, trigger role sync immediately
            if (tierResult.unlocked) {
                await role_sync_service_js_1.roleSyncService.syncUserRoles(submission.userId, interaction.client);
            }
        }
        ...
        if (submission.user.discordId) {
            try {
                ...
                if (studentUser) {
                    const isApproved = decision === 'APPROVED';
                    const studentFeedbackEmbed = isApproved
                        ? (0, embed_builder_js_1.createSuccessEmbed)(`🏆 Capstone Project Approved: Tier ${submission.project.tier}`, `Congratulations! Your Capstone Project has been evaluated and **APPROVED**!\n\n` +
                            `• **Status:** **APPROVED**\n` +
                        ...
                        : (0, embed_builder_js_1.createWarningEmbed)(`⚠️ Capstone Project Evaluation: Tier ${submission.project.tier}`, `Your instructor has evaluated your Capstone Project and provided constructive feedback:\n\n` +
        ...
        await interaction.editReply({
            embeds: [
                (0, embed_builder_js_1.createSuccessEmbed)('Project Evaluation Submitted', `Student: <@${submission.user.discordId || submission.userId}>\n` +
...
exports.studentProgressCommand = {
    data: new discord_js_1.SlashCommandBuilder()
    ...
    async execute(interaction) {
        ...
        if (!user) {
            await interaction.editReply({
                embeds: [(0, embed_builder_js_1.createWarningEmbed)('Student Not Found', 'This Discord user has not linked an Academy account.')],
            });
            return;
        }
        const summary = await progress_service_js_1.progressService.getUserProgressSummary(user.id);
        const embed = (0, embed_builder_js_1.createInfoEmbed)(`📊 Student Profile: ${targetDiscordUser.tag}`, `**Email:** \`${user.email}\`\n` +
...
//# sourceMappingURL=instructor.commands.js.map