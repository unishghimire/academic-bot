...
export const assignmentReviewCommand = {
    ...
    async execute(interaction) {
        ...
        if (decision === 'APPROVED') {
            await xpService.awardXp(submission.userId, XP_REWARDS.ASSIGNMENT_APPROVED, `Assignment Approved: ${submission.assignment.title}`, 'assignment', submissionId);
            ...
            try {
                ...
                if (studentUser) {
                    ...
                    const studentFeedbackEmbed = isApproved
                        ? createSuccessEmbed(`🎉 Assignment Approved: ${submission.assignment.title}`, `Your assignment submission has been evaluated by an instructor.\n\n` +
                            `• **Status:** **APPROVED**\n` +
                            `• **Instructor Feedback:**\n> *${feedback}*\n\n` +
                            `• **XP Earned:** **+150 XP**\n` +
                            `• **Reviewed by:** <@${interaction.user.id}>\n\n` +
                            `Run \`/progress\` or \`/course next\` in the server to view your next lesson!`)
                        : createWarningEmbed(`📝 Assignment Feedback: ${submission.assignment.title}`, `Your instructor has reviewed your assignment submission and requested revisions.\n\n` +
        ...
        await interaction.editReply({
            embeds: [
                createSuccessEmbed('Assignment Review Recorded', `Submission \`${submissionId}\` marked as **${decision}**.\nFeedback sent to student: *${feedback}*`),
            ],
        });
    },
};
export const projectReviewCommand = {
    ...
    async execute(interaction) {
        ...
        if (decision === 'APPROVED') {
            await xpService.awardXp(submission.userId, XP_REWARDS.PROJECT_APPROVED, `Tier ${submission.project.tier} Final Project Approved!`, 'project', submissionId);
        ...
        if (submission.user.discordId) {
            try {
                ...
                if (studentUser) {
                    const isApproved = decision === 'APPROVED';
                    const studentFeedbackEmbed = isApproved
                        ? createSuccessEmbed(`🏆 Capstone Project Approved: Tier ${submission.project.tier}`, `Congratulations! Your Capstone Project has been evaluated and **APPROVED**!\n\n` +
                            `• **Status:** **APPROVED**\n` +
                        ...
                        : createWarningEmbed(`⚠️ Capstone Project Evaluation: Tier ${submission.project.tier}`, `Your instructor has evaluated your Capstone Project and provided constructive feedback:\n\n` +
        ...
        await interaction.editReply({
            embeds: [
                createSuccessEmbed('Project Evaluation Submitted', `Student: <@${submission.user.discordId || submission.userId}>\n` +
...
export const studentProgressCommand = {
    ...
    async execute(interaction) {
        ...
        const embed = createInfoEmbed(`📊 Student Profile: ${targetDiscordUser.tag}`, `**Email:** \`${user.email}\`\n` +
