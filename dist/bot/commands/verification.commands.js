        ...
        if (sub === 'sync') {
            ...
            try {
                ...
                await interaction.editReply({
                    embeds: [
                        createSuccessEmbed('🔄 Database Payment Reconciliation Complete', `• **Total Approved Records Scanned:** **${syncRes.totalFound}**\n` +
        ...
        if (sub === 'list') {
            ...
            try {
                ...
                if (!payments || payments.length === 0) {
                    await interaction.editReply({
                        embeds: [
                            createInfoEmbed('🧾 Pending Payment Proofs', 'No pending payment proofs found! All submitted proofs have been reviewed.'),
                        ],
                    });
                    return;
                }
                const items = payments
                    ...
                    .join('\n');
                const embed = createInfoEmbed(`🧾 Pending Payment Proofs (${payments.length} Total)`, items +
            ...
            try {
                ...
                await interaction.editReply({
                    embeds: [
                        createSuccessEmbed('Payment Approved & Role Granted! 🎉', `Payment \`${paymentId}\` has been approved successfully.\n\n` +
                            `• **Student:** ${discordTag}\n` +
            ...
            try {
                await manualPaymentService.rejectPayment(paymentId, {
                    adminId: interaction.user.id,
                    reason,
                });
                await interaction.editReply({
                    embeds: [
                        createWarningEmbed('Payment Proof Rejected', `Payment \`${paymentId}\` has been marked as **REJECTED**.\n\n` +
                            `• **Reason:** ${reason}\n` +
    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (!methods || methods.length === 0) {
                await interaction.editReply({
                    embeds: [
                        createInfoEmbed('💳 Official Payment Methods', 'Payment methods are currently being configured by administration.\n\nPlease contact staff in `#support` for direct assistance.'),
                    ],
                });
                return;
            }
            ...
            const row = new ActionRowBuilder().addComponents(new ButtonBuilder()
                ...
                .setURL(portalUrl));
            const embed = createInfoEmbed('💳 Academy Payment Methods & Subscription Instructions', `To gain instant access to subscriber roles and private channels, make your payment through any of the verified channels below:\n\n` +
                methodBlocks.join('\n') +
                `\n📌 **After Payment:**\n` +
...
export const portalCommand = {
    ...
    async execute(interaction) {
        ...
        const row = new ActionRowBuilder().addComponents(new ButtonBuilder()
            ...
            .setStyle(ButtonStyle.Link)
            .setURL(portalUrl));
        const embed = createInfoEmbed('⚡ Academy Student Payment Portal', `Welcome to **The Elite Circle Academy** official payment & verification portal!\n\n` +
