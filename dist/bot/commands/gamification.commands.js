...
export const xpCommand = {
    ...
    async execute(interaction) {
        ...
        const embed = createInfoEmbed('✨ Your Academy XP Ledger', `**Total Balance:** **${totalXp.toLocaleString()} XP**\n` +
...
export const rankCommand = {
    ...
    async execute(interaction) {
        ...
        const totalXp = await xpService.getUserTotalXp(user.id);
        const embed = createInfoEmbed(`🎖️ Academy Standing for ${interaction.user.username}`, `**Rank:** **#${userRank}**\n` +
...
export const leaderboardCommand = {
    ...
    async execute(interaction) {
        ...
        const embed = createInfoEmbed('🏆 Academy Hall of Fame — Top Creators', lines.join('\n\n') + '\n\n*Students can opt-out of leaderboard visibility in their profile settings.*');
        await interaction.editReply({ embeds: [embed] });
    },
};
export const challengeCommand = {
    ...
    async execute(interaction) {
        ...
        const embed = createInfoEmbed('🔥 Active Academy Challenges', 'Submit your best AI video ad concepts to win bonus XP and exclusive creator badges!').addFields(fields);
        await interaction.editReply({ embeds: [embed] });
    },
};
