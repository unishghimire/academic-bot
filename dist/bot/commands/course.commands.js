...
exports.courseCommand = {
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .addStringOption(opt => opt.setName('lesson_id').setDescription('Lesson ID or title search').setRequired(true)))
        .addSubcommand(sub => sub
        .setName('complete')
        .setDescription('Mark that you have watched/studied this lesson')
        .addStringOption(opt => opt.setName('lesson').setDescription('Lesson ID or title').setRequired(true)))
        .addSubcommand(sub => sub
        .setName('quiz')
        .setDescription('Check quiz requirements and launch quiz on portal')
        .addStringOption(opt => opt.setName('lesson_id').setDescription('Lesson ID').setRequired(true)))
        .addSubcommand(sub => sub
        .setName('next')
    ...
    async execute(interaction) {
        ...
        if (sub === 'lesson') {
            ...
            const submission = lesson.assignment
                ...
                : null;
            const reqs = lesson.assignment?.requirements || ['Follow video instructions and guidelines to create your ad deliverable.'];
            ...
            const reqs = project?.requirements || ['Complete end-to-end video ad campaign with hooks, visual prompts, and creative analysis.'];
            const statusText = submission
                ? `Submission Status: **${submission.status}**${submission.feedback ? `\nInstructor Feedback: *${submission.feedback}*` : ''}`
                : 'Submission Status: ⏳ *Not submitted yet*';
...
//# sourceMappingURL=course.commands.js.map