...
export function resolveGuildRole(guild, envId, nameFallback) {
    ...
    if (nameFallback) {
        const roleList = Array.from(roles.values ? roles.values() : []);
        const roleByName = roleList.find(r => r.name?.toLowerCase() === nameFallback.toLowerCase() ||
 ...
 */
export function resolveEliteRole(guild, preferredRoleId) {
    const roles = guild.roles?.cache;
    if (!roles)
        return null;
    if (preferredRoleId) {
