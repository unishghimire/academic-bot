...
export const announceCommand = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(opt =>
      opt.setName('title').setDescription('Announcement headline / title (optional)').setRequired(false)
    )
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const chosenChannel = interaction.options.getChannel('channel') as TextChannel | null;
      ...
      await interaction.editReply({
        embeds: [
          createSuccessEmbed(
            ...
            `Successfully posted your announcement to <#${targetChannel.id}>!\n\n` +
            `• **Title:** ${title}\n` +
