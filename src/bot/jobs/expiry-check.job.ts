 ...
 */
export async function runExpirySweep(client: Client): Promise<void> {
        ...
        if (!localStore.hasExpiredNoticeBeenSent(u.id, u.expiresAt)) {
          try {
            ...
            if (discordUser) {
              ...
              logger.info(
                { userId: u.id, discordId: u.discordId },
                'Delivered subscription expired DM with portal reactivation link'
              );
            }
          } catch {
    ...
    for (const u of expiringUsers) {
      ...
      try {
        ...
        if (discordUser) {
          ...
          logger.info(
            { userId: u.id, discordId: u.discordId, daysLeft },
            'Delivered 3-day advance subscription expiration reminder DM'
          );
        }
      } catch {
        // Ignore DM failure if user has closed DMs
