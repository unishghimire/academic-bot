 ...
 */
export async function runMeetingLiveCheck(client: Client): Promise<void> {
  ...
  try {
    ...
    if (!dueMeetings || dueMeetings.length === 0) return;

    for (const meeting of dueMeetings) {
      try {
        logger.info(
          { meetingId: meeting.id, title: meeting.title },
          '⏰ Scheduled meeting start time reached! Provisioning live voice room and broadcasting announcement...'
        );
        ...
        if (announcementChannel) {
          const liveEmbed = new EmbedBuilder()
            ...
            .setDescription(
              `The scheduled live class is **now open and starting**!\n\n` +
              `• **Topic:** ${meeting.topic}\n` +
 ...
 */
export function initMeetingLiveJob(client: Client): void {
