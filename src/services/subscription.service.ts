...
export class SubscriptionService {
   ...
   */
  async sweepExpiredSubscriptionsDetailed(): Promise<ExpiredUserSummary[]> {
    const now = new Date();
    const sweptUsers: ExpiredUserSummary[] = [];
    ...
    try {
      if (this.db?.user?.findMany) {
        ...
        for (const user of expiredUsers) {
          ...
          seenIds.add(user.id);
          if (user.discordId) seenIds.add(user.discordId);
    ...
    try {
      ...
      for (const lu of localUsers) {
        if (
          lu.subscriptionStatus === SubscriptionStatus.ACTIVE &&
          ...
          if (!seenIds.has(lu.id) && (!lu.discordId || !seenIds.has(lu.discordId))) {
            seenIds.add(lu.id);
            if (lu.discordId) seenIds.add(lu.discordId);
          ...
          for (const rec of verifs) {
            ...
            if (expiresAt.getTime() <= now.getTime()) {
              ...
              if (!seenIds.has(rec.id) && (!rec.discord_id || !seenIds.has(rec.discord_id))) {
                seenIds.add(rec.id);
                if (rec.discord_id) seenIds.add(rec.discord_id);
   ...
   */
  async findExpiringUsers(withinHours: number): Promise<ExpiringUserSummary[]> {
    const now = new Date();
    const windowEnd = new Date(now.getTime() + withinHours * 60 * 60 * 1000);
    const expiringUsers: ExpiringUserSummary[] = [];
      ...
      if (this.db?.user?.findMany) {
        ...
        for (const u of users) {
          if (u.subscriptionExpiresAt) {
            ...
            if (u.discordId) seenDiscordIds.add(u.discordId);
        ...
        if (
          ...
          if (expDate > now && expDate <= windowEnd) {
            if (!lu.discordId || !seenDiscordIds.has(lu.discordId)) {
              if (lu.discordId) seenDiscordIds.add(lu.discordId);
              const hoursRemaining = Math.max(0, (expDate.getTime() - now.getTime()) / (1000 * 60 * 60));
              expiringUsers.push({
                id: lu.id,
                discordId: lu.discordId || null,
          ...
          for (const rec of verifs) {
            ...
            if (expDate > now && expDate <= windowEnd) {
              if (!rec.discord_id || !seenDiscordIds.has(rec.discord_id)) {
                if (rec.discord_id) seenDiscordIds.add(rec.discord_id);
                const hoursRemaining = Math.max(0, (expDate.getTime() - now.getTime()) / (1000 * 60 * 60));
                expiringUsers.push({
                  id: rec.id,
                  discordId: rec.discord_id || null,
